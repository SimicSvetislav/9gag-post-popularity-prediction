...
METADATA_FILE = 'metadata_patterns.csv'
IMAGES_FILE = '../../scraped/'
DATABASE_IMAGES_FILE = '../../scraped_database_tags_new/'

def get_labels(file=METADATA_FILE, evaluation=False):
    ...
    with open(file, 'r') as metadata:
        ...
        print("Files in {0} : {1}".format(IMAGES_FILE, samples))
        ...
        # for x in range(1, len(dataset)):
            img_path = IMAGES_FILE + dataset[x][1] + "/" + dataset[x][0]
...
def get_data_for_generator(file=METADATA_FILE):
    img_files = []
    labels = []
...
def load_post_images(file=DATABASE_IMAGES_FILE):
